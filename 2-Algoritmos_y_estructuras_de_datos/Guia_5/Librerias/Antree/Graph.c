...
/**
    ...
    Paso un vector donde se va a almacenar la lista de vertices adyacentes al vertice que le mandamos.
    Devuelve la longitud del vector.
**/

int listAdjVertex(graph *g, int v[MAX], int indexV)
{
    int sizeL = 0;
    for(int i = 0; i < graphSize(g); i++)
    {
...
/**
    ...
    -Si el grafo es orientado debe cumplir con solo una de las siguientes condiciones, siendo u y v dos vertices:
        *Es un digrafo, es decir que existe una arista (u, v) o una arista (u, v), pero no ambas a la vez.
...
int isSimpleGraph(graph *g)
{
    ...
    for(int i = 0; i < graphSize(g); i++)
    {
        if(numberOfArcs(g, i) > 1 || graphExistArc(g, i, i))
        {
            result = 0;
        }
    }
