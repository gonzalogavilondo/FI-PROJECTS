...
void insertValues(btn **root, int values[], int count)
{
    ...
    {
        //printf("\nAgregando: %d\n", values[i]);
        if(!insertValueSBT(root, values[i])) //Lo inserte como SBT ya que de igual manera esta balanceado
        {
...
void removeValues(btn **root, int deletee[], int countd)
{
    for (int i = 0; i < countd; i++)
    {
        printf("\nEliminando: %d\n", deletee[i]);
        removeNodeSBTRep(findNodeSBT(root, deletee[i])); //Eliminamos el valor dado por deletee
    }
}
